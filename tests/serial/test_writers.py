...
class TestWriteInt64(IntWriterContract):
    ...
    def match_error_message(self) -> str:
        return (
            fr"^'>q' format requires {self.lower_limit} <= number <= {self.upper_limit}$"
...
class TestWriteUint16(IntWriterContract):
    ...
    def lower_limit_error_message(self) -> str:
        return (
            fr"^'>H' format requires {self.lower_limit} <= number <= {self.upper_limit}$"
            if sys.version_info >= (3, 12)
            else
...
class TestWriteUint32(IntWriterContract):
    ...
    def lower_limit_error_message(self) -> str:
        return (
            fr"^'>I' format requires {self.lower_limit} <= number <= {self.upper_limit}$"
            if sys.version_info >= (3, 12)
            else
            r"^argument out of range$"
        )
