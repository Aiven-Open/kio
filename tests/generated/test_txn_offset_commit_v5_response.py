...
from kio.schema.txn_offset_commit.v5.response import TxnOffsetCommitResponse
from kio.schema.txn_offset_commit.v5.response import TxnOffsetCommitResponsePartition
from kio.schema.txn_offset_commit.v5.response import TxnOffsetCommitResponseTopic
...
def test_txn_offset_commit_response_partition_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        result, _ = read_txn_offset_commit_response_partition(
            buffer.getvalue(),
            0,
        )
...
def test_txn_offset_commit_response_topic_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_txn_offset_commit_response_topic(
            buffer.getvalue(),
            0,
        )
...
def test_txn_offset_commit_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_txn_offset_commit_response(
            buffer.getvalue(),
            0,
        )
