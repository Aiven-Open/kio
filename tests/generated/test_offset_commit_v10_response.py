...
from kio.schema.offset_commit.v10.response import OffsetCommitResponse
from kio.schema.offset_commit.v10.response import OffsetCommitResponsePartition
from kio.schema.offset_commit.v10.response import OffsetCommitResponseTopic
...
def test_offset_commit_response_partition_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        result, _ = read_offset_commit_response_partition(
            buffer.getvalue(),
            0,
        )
...
def test_offset_commit_response_topic_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_offset_commit_response_topic(
            buffer.getvalue(),
            0,
        )
...
def test_offset_commit_response_roundtrip(instance: OffsetCommitResponse) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_offset_commit_response(
            buffer.getvalue(),
            0,
        )
