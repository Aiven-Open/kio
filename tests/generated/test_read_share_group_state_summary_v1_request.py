...
from kio.schema.read_share_group_state_summary.v1.request import PartitionData
from kio.schema.read_share_group_state_summary.v1.request import (
    ReadShareGroupStateSummaryRequest,
)
from kio.schema.read_share_group_state_summary.v1.request import ReadStateSummaryData
...
def test_read_state_summary_data_roundtrip(instance: ReadStateSummaryData) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_read_state_summary_data(
            buffer.getvalue(),
            0,
        )
...
def test_read_share_group_state_summary_request_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_read_share_group_state_summary_request(
            buffer.getvalue(),
            0,
        )
