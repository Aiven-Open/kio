...
from kio.schema.share_acknowledge.v1.response import LeaderIdAndEpoch
from kio.schema.share_acknowledge.v1.response import NodeEndpoint
from kio.schema.share_acknowledge.v1.response import PartitionData
from kio.schema.share_acknowledge.v1.response import ShareAcknowledgeResponse
from kio.schema.share_acknowledge.v1.response import ShareAcknowledgeTopicResponse
...
def test_share_acknowledge_topic_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_share_acknowledge_topic_response(
            buffer.getvalue(),
            0,
        )
...
def test_share_acknowledge_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_share_acknowledge_response(
            buffer.getvalue(),
            0,
        )
