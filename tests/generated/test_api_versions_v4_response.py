...
from kio.schema.api_versions.v4.response import ApiVersion
from kio.schema.api_versions.v4.response import ApiVersionsResponse
from kio.schema.api_versions.v4.response import FinalizedFeatureKey
from kio.schema.api_versions.v4.response import SupportedFeatureKey
...
def test_supported_feature_key_roundtrip(instance: SupportedFeatureKey) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_supported_feature_key(
            buffer.getvalue(),
            0,
        )
...
def test_finalized_feature_key_roundtrip(instance: FinalizedFeatureKey) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_finalized_feature_key(
            buffer.getvalue(),
            0,
        )
...
def test_api_versions_response_roundtrip(instance: ApiVersionsResponse) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_api_versions_response(
            buffer.getvalue(),
            0,
        )
