...
from kio.schema.write_share_group_state.v1.response import PartitionResult
from kio.schema.write_share_group_state.v1.response import WriteShareGroupStateResponse
from kio.schema.write_share_group_state.v1.response import WriteStateResult
...
def test_write_state_result_roundtrip(instance: WriteStateResult) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_write_state_result(
            buffer.getvalue(),
            0,
        )
...
def test_write_share_group_state_response_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_write_share_group_state_response(
            buffer.getvalue(),
            0,
        )
