...
from kio.schema.write_share_group_state.v1.request import PartitionData
from kio.schema.write_share_group_state.v1.request import StateBatch
from kio.schema.write_share_group_state.v1.request import WriteShareGroupStateRequest
from kio.schema.write_share_group_state.v1.request import WriteStateData
...
def test_write_state_data_roundtrip(instance: WriteStateData) -> None:
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_write_state_data(
            buffer.getvalue(),
            0,
        )
...
def test_write_share_group_state_request_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_write_share_group_state_request(
            buffer.getvalue(),
            0,
        )
