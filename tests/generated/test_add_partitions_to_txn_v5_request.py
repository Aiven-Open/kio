...
from kio.schema.add_partitions_to_txn.v5.request import AddPartitionsToTxnRequest
from kio.schema.add_partitions_to_txn.v5.request import AddPartitionsToTxnTopic
...
def test_add_partitions_to_txn_topic_roundtrip(
    ...
    with setup_buffer() as buffer:
        ...
        result, _ = read_add_partitions_to_txn_topic(
            buffer.getvalue(),
            0,
        )
...
def test_add_partitions_to_txn_transaction_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_add_partitions_to_txn_transaction(
            buffer.getvalue(),
            0,
        )
...
def test_add_partitions_to_txn_request_roundtrip(
    ...
    with setup_buffer() as buffer:
        writer(buffer, instance)
        result, _ = read_add_partitions_to_txn_request(
            buffer.getvalue(),
            0,
        )
